...
public class PartitionAssignment {
    ...
    private final Map<String, List<Integer>> assignments = new LinkedHashMap<>();

    public PartitionAssignment(List<String> members, int partitionCount) {
    ...
    public List<Integer> partitionsFor(String memberId) {
        if (memberId == null || memberId.isBlank()) {
            throw new IllegalArgumentException("Member ID cannot be blank");
        }
        List<Integer> partitions = assignments.get(memberId);
        if (partitions == null) {
            return List.of();
        }
    ...
    public Map<String, List<Integer>> assignments() {
        ...
        assignments.forEach((member, partitions) -> copy.put(member, List.copyOf(partitions)));
