...
public class LeaveGroupDispatcherTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("=== LEAVE GROUP DISPATCHER TEST ===");
        ...
        Path offsetDirectory = Files.createTempDirectory("leave-group-test");
        ...
        if (response.status() != Response.SUCCESS) {
            throw new AssertionError("LEAVE_GROUP dispatch failed");
        }
        ...
        if (!responsePayload.memberId().equals("consumer-A")) {
            throw new AssertionError("Wrong member in response");
        }
        ...
        if (!groupManager.groupExists("orders-group")) {
            throw new AssertionError("Consumer group should still exist");
        }

        System.out.println();
        System.out.println("LEAVE_GROUP dispatcher " + "verified successfully!");
    }
}