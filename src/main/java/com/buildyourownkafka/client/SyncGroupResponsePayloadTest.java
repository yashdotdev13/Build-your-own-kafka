...
public class SyncGroupResponsePayloadTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("=== SYNC GROUP RESPONSE PAYLOAD TEST ===");
        SyncGroupResponsePayload original = new SyncGroupResponsePayload("consumer-A", 0, List.of(0, 2));
        byte[] encoded = original.encode();
        System.out.println("Encoded payload size: " + encoded.length);
        SyncGroupResponsePayload decoded = SyncGroupResponsePayload.decode(encoded);
        ...
        if (!decoded.partitions().equals(List.of(0, 2))) {
            throw new AssertionError("Partitions mismatch");
        }
        System.out.println();
        System.out.println("SYNC_GROUP response payload " + "verified successfully!");
    }
}