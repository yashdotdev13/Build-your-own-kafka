...
public class LeaveGroupRequestHandlerTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("=== LEAVE GROUP REQUEST HANDLER TEST ===");
        ...
        System.out.println("Members before leave: " + groupManager.getGroup("orders-group").memberCount());
        LeaveGroupRequestHandler leaveHandler = new LeaveGroupRequestHandler(coordinator);
        ...
        Response response = leaveHandler.handle(leaveRequest);
        System.out.println("Response correlation ID: " + response.correlationId());
        System.out.println("Response status: " + response.status());
        if (response.status() != Response.SUCCESS) {
            throw new AssertionError("LEAVE_GROUP request failed");
        }

        LeaveGroupResponsePayload responsePayload = LeaveGroupResponsePayload.decode(response.payload());
        System.out.println("Left member: " + responsePayload.memberId());

        if (!responsePayload.memberId().equals("consumer-A")) {

            throw new AssertionError("Incorrect member ID in response");
        }
        if (!groupManager.groupExists("orders-group")) {
            throw new AssertionError("Group should still exist");
        }

        if (groupManager.getGroup("orders-group").memberCount() != 1) {
            throw new AssertionError("Consumer-A was not removed");
        }
        ...
        PartitionAssignment assignment = coordinator.getAssignment("orders-group");
        System.out.println("Remaining assignment: " + assignment.assignments());
        if (assignment == null) {
            throw new AssertionError("Assignment should exist");
        }
        System.out.println();
        System.out.println("LEAVE_GROUP request handler " + "verified successfully!");
    }
}