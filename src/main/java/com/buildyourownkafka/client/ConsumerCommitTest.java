...
public class ConsumerCommitTest {
    ...
    public static void main(String[] args)
        ...
        System.out.println(
                "Current offset after poll: "
                        + consumer1.currentOffset()
        );

        /*
         * We must actually consume something.
         */

        if (records.isEmpty()) {

            throw new RuntimeException(
                    "Consumer did not receive any records"
            );
        }
        ...
        if (recoveredOffset
            ...
            throw new RuntimeException(
                    "Offset recovery failed. Expected "
                            + committedOffset
                            + " but got "
                            + recoveredOffset
            );
        ...
        List<Record> recoveredRecords =
                consumer2.poll();

        System.out.println(
                "Records received after restart: "
                        + recoveredRecords.size()
        );

        /*
         * We should NOT receive the already committed
         * records again.
         */
        ...
        if (!recoveredRecords.isEmpty()) {
            ...
            throw new RuntimeException(
                    "Consumer received already committed records"
            );
        }

        System.out.println(
                "Current offset after restart poll: "
                        + consumer2.currentOffset()
        );
        ...
        System.out.println(
                "Consumer offset recovery verified!"
        );
        ...
        System.out.println(
                "Already committed records were not replayed."
        );
