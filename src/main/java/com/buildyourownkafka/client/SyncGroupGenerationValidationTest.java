...
public class SyncGroupGenerationValidationTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println(
                "=== SYNC GROUP GENERATION VALIDATION TEST ==="
        );
        ...
        SyncGroupRequestHandler syncHandler =
                new SyncGroupRequestHandler(
                        coordinator
                );
        ...
        SyncGroupRequestPayload stalePayload =
        ...
        System.out.println(
                "Stale generation response status: "
                        + staleResponse.status()
        );

        System.out.println(
                "Stale generation response message: "
                        + new String(
                        staleResponse.payload()
                )
        );
        ...
        if (staleResponse.status()
                != Response.ERROR) {

            throw new AssertionError(
                    "Stale generation should return ERROR"
            );
        }
        ...
        if (staleResponse.correlationId() != 100) {
            ...
            throw new AssertionError(
                    "Correlation ID mismatch for stale request"
            );
        }
        ...
        SyncGroupRequestPayload validPayload =
                new SyncGroupRequestPayload(
                        "orders-group",
                        "consumer-A",
                        2
        ...
        System.out.println(
                "Current generation response status: "
        ...
        if (validResponse.correlationId() != 101) {
            ...
            throw new AssertionError(
                    "Correlation ID mismatch for valid request"
            );
        }

        SyncGroupResponsePayload responsePayload =
                SyncGroupResponsePayload.decode(
                        validResponse.payload()
                );

        System.out.println(
                "Consumer-A assignment: "
        ...
        if (responsePayload.partitions().isEmpty()) {
            ...
            throw new AssertionError(
                    "Consumer-A should have partitions"
            );
        }
        ...
        System.out.println(
                "SYNC_GROUP generation validation "
