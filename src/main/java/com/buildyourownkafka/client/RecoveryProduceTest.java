...
public class RecoveryProduceTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (Socket socket = new Socket("localhost", 9092)) {
            ...
            ProducePayload payload = new ProducePayload("orders", 1, "order-after-restart".getBytes(StandardCharsets.UTF_8));
            Request request = new Request(Request.PRODUCE, (short) 1, 300, payload.encode());
            ...
            if (response.status() == Response.SUCCESS) {
                ProduceResponsePayload produceResponse = ProduceResponsePayload.decode(response.payload());
                System.out.println("Recovered next offset → " + produceResponse.offset());
            } else {
