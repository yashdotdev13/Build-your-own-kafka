...
public class HeartbeatDispatcherTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Path tempDirectory =
                Files.createTempDirectory("heartbeat-dispatcher-test");

        Path topicStorage =
                tempDirectory.resolve("topics");

        Path offsetStorage =
                tempDirectory.resolve("offsets");

        Files.createDirectories(topicStorage);
        Files.createDirectories(offsetStorage);

        TopicManager topicManager =
                new TopicManager(topicStorage);

        ConsumerOffsetStore consumerOffsetStore =
                new ConsumerOffsetStore(offsetStorage);
