    ...
    public byte[] encode() {
        ...
        try {
            ...
            throw new IllegalStateException(
                    "Failed to encode CREATE_TOPIC payload",
                    e
            );
        }
    }

    public static CreateTopicPayload decode(
            byte[] payload
    ) {
        ...
        try {
            ...
            if (input.available() != 0) {
                throw new IllegalArgumentException(
                        "Unexpected bytes in CREATE_TOPIC payload"
                );
            }

            return new CreateTopicPayload(
                    topicName,
                    partitionCount
            );
